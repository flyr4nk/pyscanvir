...
""" VirusTotal File Scan
    ...
    Based in the work of Xiaokui Shu
...
class VT_Response_Send(object):
    HTTP_OK = 200

    def __init__(self, fullpath_filename, res):
        self.status_code = res.status_code
        self.fullpath_filename = fullpath_filename
        self.filename = os.path.basename(fullpath_filename)
        self.res_text = res.text
        if res.status_code == self.HTTP_OK:
            resmap = json.loads(res.text)
            self.response_code = resmap["response_code"]
            self.scan_id = resmap["scan_id"]
            self.verbose_msg = resmap["verbose_msg"]
        else:
...
class VT_Response_Received(object):
    HTTP_OK = 200
    ...
    def __init__(self, res):
        ...
        if res.status_code == self.HTTP_OK:
            ...
            if self.response_code == 1:
                self.positives = resmap["positives"]
                self.total = resmap["total"]
        else:
            self.response_code = -1
            self.scan_id = ""
            self.verbose_msg = ""
...
class VirusTotal(object):
    ...
    def send_files_withresponse(self, filenames):
        """
        Send files to scan

        @param filenames: list of target files

        return: a list containing the response of every target file
        """
        url = self.URL_BASE + "file/scan"
        attr = {"apikey": self.apikey}
        ...
        for filename in filenames:
            files = {"file": open(filename, 'rb')}
            res = requests.post(url, data=attr, files=files)
            vt_res = VT_Response_Send(filename, res)
            responses.append(vt_res)

            if vt_res.status_code == self.HTTP_OK:
                if not self.is_verboselog:
                    self.logger.info("sent: %s, HTTP: %d, response_code: %d, scan_id: %s",
                            vt_res.filename, vt_res.status_code, vt_res.response_code, vt_res.scan_id)
                else:
                    self.logger.info("sent: %s, HTTP: %d, content: %s", vt_res.filename, vt_res.status_code, vt_res.text)
            else:
                self.logger.warning("sent: %s, HTTP: %d", vt_res.filename, vt_res.status_code)
    ...
    def retrieve_report_withresponse(self, chksum):
        """
        Retrieve Report for the file checksum
        ...
        res = requests.post(url, data=params)
        self.has_sent_retrieve_req = True
        vt_res = VT_Response_Received(res)
        return vt_res
